...
const addRow = ()=>{
    ...
    const columns = table.rows[0].cells.length;
    for(let i = 0; i < columns; i++)row.insertCell(-1);
    updateButtonState();
};
const removeRow = ()=>{
...
const addColumn = ()=>{
    ...
    for(let i = 0; i < rows.length; i++)rows[i].insertCell(columnIndex);
    updateButtonState();
};
const removeColumn = ()=>{
    const rows = table.rows;
    ...
    if (columnIndex >= MIN_ROWS_COLUMNS - 1) {
        for(let i = 0; i < rows.length; i++)rows[i].deleteCell(columnIndex);
        updateButtonState();
    }
};
